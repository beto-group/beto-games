...
< 400px), standard titles often clip. Use: - **`maxWidth: 85vw`** + **`padding: 0 20px`** on the root container. -
  **Aggressive Clamps**: `fontSize: clamp(18px, 6vw, 120px)`. - **Gutters**: Mandatory horizontal padding ensures
  the "Character Clipping" failure is physically impossible. ## 🧭 Navigation & Layout Flow - **Sticky vs. Fixed**:
...
- **Obsidian-First Markdown**: (NEW) Use standard `.md` extensions for all content and agent documentation. This ensures 100% Obsidian compatibility (backlinks, tags, previews) while remaining fully manageable via Keystatic.
  ...
  View**: Test changes in the standard browser (`npm run dev`) only after verifying them in Datacore. ## 🛠️ Debugging -
  **DebugManager**: Use `debugManager.log()` for environment-aware logging. - **Console Versioning**: Check for the 💎
  (Datacore) or 🌐 (Website) emoji to confirm which environment is logged. - **Evaluation Logs**: Ensure components log
  their evaluation state to detect loading failures early. ## ⚠️ Common Pitfalls - **Missing Exports**: Ensure the shim
...
## ⚙️ Background Tasks & Command Execution
...
## 🌉 Hybrid Compatibility (Next.js + Datacore)
...
```
WebsiteBuilder → MarkdownRenderer → DatacoreShim → AsyncFactory → InnerComponent → ChildComponent
```
...
```javascript
// WebsiteBuilder.jsx
<MarkdownRenderer content="{component: RetroMorphGame}" setCurrentPage={setActiveTab} // ← Pass the prop ... />
```
...
// ↑ Capture from factory props with alias
...
## ☁️ Integrations (Cloudflare & GitHub)
...
| `external` | boolean | Marks as external (shows ↗ arrow) |
...
2. Run `npm run dev` — all sizes regenerate automatically.
...
enough — the page body itself can still scroll. Two-part fix required:
...
**2. Body scroll-lock via `<style>
  ` tag** (prevents iOS touch/momentum scroll): ```jsx <style> {
...
**Key rules**:
- Use `100dvh` not `100vh` or `100svh` — adapts when mobile browser chrome shows/hides.
- `minHeight` will NOT work — always use `height` for scroll elimination.
...
through the entire WebsiteBuilder chain. To avoid the "Slow Button" problem where the prompt event
fires before React hydrates, use the **Early Capture Pattern**.

### Required Files
| File | Purpose |
|------|---------|
| `public/sw.js` | Service worker — required for Android Chrome to offer install |
| `src/app/layout.jsx` | Contains the Early Capture inline script |
| `src/hooks/usePWAInstall.jsx` | Standalone hook for reuse |
| `public/manifest.json` | Must have `display: "standalone"` and icon entries |

### The Early Capture Pattern (layout.jsx)
Add a synchronous `
<script>` in the ` < body > ` of `layout.jsx` to capture the event before React mounts:
...
```javascript
    ...
    (function() {
        ...
        // Capture event before React mounts
...
Check for the captured prompt immediately on mount for a "Zero-Delay" UI:
...
```javascript
  function HeroSection({ STYLES, buttonText, dc }) {
    ...
    _useEffect(() => {
      ...
      // ⚡ FAST PATH: prompt was already captured by layout.jsx
      if (window.__pwaInstallPrompt) {
        setDeferredPrompt(window.__pwaInstallPrompt);
        setCanInstall(true);
      }

      // FALLBACK: listen for the custom ready event
      const onReady = (e) => {
...
| iOS Safari | Always available (no event) | Show manual Share → "Add to Home Screen" modal |
| Already installed | `display - mode: standalone` | Button disabled / shows "✓ INSTALLED" |
| Desktop | No `beforeinstallprompt` | Show default button text |

### Button Label Pattern
```javascript
  const installLabel = isInstalled ? '✓ INSTALLED'
...
- Instruction to tap the Share button (⎋) in Safari
...
- The service worker (`sw.js`) must be at the root ` / sw.js` — not a subdirectory
...
## 18. Mobile Audio / Speech Synthesis Unlock

### Overview
Mobile browsers (especially iOS Safari) block audio and `window.speechSynthesis` until a direct user gesture (click/tap) unlocks them. The most robust unlock involves both Web Audio and Speech Synthesis priming.

### The "Extreme Unlock" Pattern
To ensure audio works, prime both engines **synchronously** inside the gesture handler:
...
```javascript
  const initAudio = () => {
    ...
    if (window.speechSynthesis) {
      ...
      // MUST speak an audible character (like " ") with volume > 0
