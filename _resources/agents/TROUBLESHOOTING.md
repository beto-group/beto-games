...
### 🔴 `Error: File not found: .../Component.jsx` (Datacore Require)
...
### 🟡 `position: fixed` Header Jumps to Top of Window
...
### 🟡 Local Dependencies fail to load in Shim
...
### 🟡 `Module not found: Can't resolve ...` in .generated.jsx
...
<body>`. Ensure these are accounted for or suppressed to avoid hydration mismatches that crash the CMS UI.
    ...
    **The Fix (2-Part):**
    ...
    <div style={{ ...STYLES.container, height: '100vh' , // ← Establishes the height chain display: 'flex' ,
        ...
        <div style={{ width: '100%' , flex: 1, // ← Fills remaining space after navbar display: 'flex' ,
            flexDirection: 'column' , overflow: 'hidden' }}>
            <Shim ... />
        </div>
        ```
        ...
        **Why This Works:**
        - `height: 100vh` on root → flex children have a defined space to fill
        - `flex: 1` on children → fills remaining space after navbar
        - `overflow: hidden` → prevents scrolling (matches Datacore behavior)

        - `overflow: hidden` → prevents scrolling (matches Datacore behavior)

        ### 🔴 Canvas Only Draws in Top-Left Corner After Resize
        ...
        **Solution**: Add all sizing constants to the `useEffect` dependency array that manages the render loop.
        ...
        }, [CANVAS_WIDTH, CANVAS_HEIGHT, GRID_SIZE]); // ← MUST have these
        ```

        ### 🟡 Internal Dependencies Fail via dc.require on Web
        ...
        **Cause**: Editing `.generated.jsx` files directly instead of their source files. The shim regenerates these on
        every build.
        **Critical Rule**:
        - ❌ **Don't edit**: `MarkdownRenderer.generated.jsx`, `shim.generated.jsx`
        - ✅ **Do edit**: `MarkdownRenderer.jsx`, `shim.js`
        ...
        ### 🔴 Component Shows "Loading..." Forever / Hooks are Undefined
        ...
        **Verification**: After running `npm run shim`, check the `.generated.jsx` file:
        ...
        ### 🔴 Malformed Imports in Generated Files
        ...
        **Verification**: After `npm run shim`, check `src/index.generated.jsx`:
        ...
        ### 🔴 Homepage Shows "# Loading..." Instead of RetroMorphGame
        ...
        **Solution**: Prefix external link keys with `ext-` to ensure uniqueness:
        ...
        <div key={navKey} ...>
            ...
            **Result**:
            - Internal `HOME` → key = `HOME`
            - External `HOME` → key = `ext-HOME`

            ---

            ## 11. YAML Comments Rendered as Menu Items

            ### 🟡 Burger Menu Shows "INTERNAL PAGES" or "# COMMENT" as Items
            ...
            **Note**: The parser DOES support multi-line item definitions (label, value, url, external, separator). Just
            ...
            ### 🔴 `Polling failed Error: Project not found`
            ...
            **Solution**:
            ...
            ### 🟡 GitHub Push Fails with "nothing to commit"
            ...
            **Solution**: The `initializeAndPush` function now checks `git status --porcelain` before attempting to
            ...
            ### 🔴 `/admin` Redirects to `/keystatic` or Sidebar Links Point to `/keystatic`
            ...
            **Solutions**:
            ...
            ### 🔴 Silent Build Failure (Exit Code 1, No Output)
            ...
            **Solution**:
            ...
            ### 🔴 `Page "..." cannot use both "use client" and export function "generateStaticParams()"`
            ...
            **Solution**:
            ...
            ### 🔴 `Page "..." is missing "generateStaticParams()"` (Optional Catch-all)
            ...
            **Solution**: Return an array containing an empty params object to generate the base path:
            ...
            ### 🔴 Dynamic API Routes in Static Export
            ...
            **Solution**:
            ...
            ## 15. dc.Icon Fallback (ⓘ) on Web — Missing Icons in Shim

            ### 🟡 Icons Show as Placeholder `ⓘ` on Web (Not in Datacore)
            **Symptoms**: Game or component icons (e.g., eye, volume-2, bar-chart-2, x) render as a small boxed `ⓘ`
            character on the website, but display correctly in Datacore/Obsidian.
            **Cause**: `dc.Icon` is a native Datacore API component that renders Lucide icons. On the web, `shim.js`
            provides a fallback `Icon` component with a local SVG dictionary. If the requested icon name is not in that
            dictionary, it falls back to the placeholder.

            **Solution**: Add the missing icon's Lucide SVG path to the `icons` dictionary in `src/web/shim.js`:
            ```javascript
            // src/web/shim.js — Inside the icons dictionary
            'eye': (
            ...
            // ... etc. Copy SVG path from lucide.dev for the icon name.
            ```

            **Currently Registered Icons** (as of 2026-02):
            `zap`, `rotate-ccw`, `help-circle`, `eye`, `volume-2`, `bar-chart-2`, `x`, `chevron-left`,
            `chevron-right`, `refresh-cw`

            **Rule**: Whenever a new component uses a new `dc.Icon` icon name, add its SVG to `shim.js`. After
            editing `shim.js`, run `npm run shim` to regenerate `shim.generated.jsx`.

            **Also**: The `Icon` shim now accepts a `size` prop (numeric pixels) which is passed through to the
            SVG's `width` and `height` attributes, matching the Datacore `dc.Icon` API.

            ---

            ## 16. Favicon Shows Old/Wrong Icon After Generation

            ### 🟡 Browser Tab Shows Stale Favicon After Running generate-favicons
            **Symptoms**: `npm run dev` regenerates favicons from `public/logo.png`, but the browser tab still
            shows the old icon.
            **Cause**: Browsers aggressively cache favicons. A hard reload is required to force the browser to
            re-fetch.
            **Solution**:
            - `Cmd + Shift + R` (Mac) / `Ctrl + Shift + R` (Windows) for a hard reload.
            - If that doesn't work: DevTools → Network tab → check "Disable cache" → reload.

            **Also Check**: If there is a stale `public/favicon.png` (different from `favicon.ico` /
            `favicon-32x32.png`),
            browsers may auto-discover and prefer it. Delete any stale favicon variants from /public that are not
            part of the `generate-favicons.js` output set.

            ---

            ## 17. Embedded Game Scroll Issues (Mobile + Desktop)

            ### 🟡 Unwanted Vertical Scrollbar Appears Inside Game Components
            **Symptoms**: The IQGame (or other full-screen game embedded in the Next.js website) triggers a vertical
            scrollbar on mobile or desktop. Tweaking padding/margin doesn't help reliably.
            **Root Cause**: The game component is rendered *inside a scrollable webpage*. Setting `overflow: hidden`
            on the game's root div is not enough because the *page* itself can scroll.

            **Two-Part Solution**:

            **1. Make the game container `position: fixed`** (owns the viewport entirely):
            ```javascript
            // styles.jsx
            container: {
            position: 'fixed', // Owns the entire viewport
            top: 0,
            left: 0,
            width: '100vw',
            ...
            **2. Inject body scroll-lock CSS** via a `<style>
                ` tag inside the component: ```jsx // IQGame.jsx (or any full-screen game)
            ...
            **Why this works**:
            - `position: fixed` takes the game out of the document flow entirely — the page can't scroll because
            the game visually covers it, and there's no layout above/below to scroll to.
            - The body lock is a safety net for touch/momentum scroll on iOS.
            - `100dvh` (dynamic viewport height) correctly accounts for the mobile browser's address bar being
            shown/hidden — use this instead of `100vh` or `100svh` for games.

            **Note**: Using `minHeight` instead of `height` on the container will still allow scrolling if content
            overflows. Always use `height: 100dvh` + `overflow: hidden` for true scroll elimination.
            ---

            ## 18. PWA Install Button Does Nothing on Mobile

            ### 🟡 "Add to Home Screen" Button Triggers But Nothing Downloads/Installs
            **Symptoms**: The hero button is tapped on mobile but nothing happens — no install prompt appears,
            no download occurs.
            **Root Cause (most common)**: The button has no `onClick` handler, OR the `beforeinstallprompt`
            event was never captured because there is no service worker registered.

            **PWA Install requires ALL THREE of these**:
            1. A valid `manifest.json` with `"display": "standalone"`, `name`, and icon entries
            2. A **Service Worker** registered at `/sw.js` (served from root)
            3. HTTPS (localhost is fine for dev; production requires it)

            **Implementation**: See `BEST_PRACTICES.md § 16` for the complete self-contained component pattern.
            ...
            **Debugging beforeinstallprompt on Android**:
            In Chrome DevTools → Application tab → Manifest → check for install criteria errors.
            Common errors: missing icons, missing `start_url`, or not HTTPS.

            **iOS standalone detection**:
            ```javascript
            // WRONG: matchMedia alone doesn't detect iOS standalone
            ...
            ## 19. No Audio/Sounds on Mobile (iOS/Android)
